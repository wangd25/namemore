...
- Phase 7C1 is complete locally on `codex/phase-7c-draft-review`. A fifteenth forward-only migration adds owner-only draft listing/editing and an irreversible `review-requested`/`pending` submission transition. The responsive three-region workspace, no-store list/create/update/submit handlers, 114 tests, category-draft hostile-client suite, lint, typecheck, build, bundle audit, live grants/constraint inspection, advisors, and desktop/mobile browser flow pass. Publishing and exact-source protected Preview verification are the remaining handoff steps for this slice.
...
| 7. General category studio and discovery | **In progress** | Phases 7A–7C1 ship editable discovery, real aggregate cards, reviewed general practice, and owner-only draft review requests; reviewer authority, publishing, moderation, corrections, and lifecycle workflows remain. |
...
**Status: In progress.** Phases 7A and 7B are published and Preview-verified. Phase 7C1 is complete locally on `codex/phase-7c-draft-review` and awaits its exact-source protected Preview. This phase makes the product broadly category-first without allowing unreviewed generation into competitive play.
...
- Phase 7C1 is complete locally on `codex/phase-7c-draft-review`; application publication and exact-source protected Preview verification are the remaining steps for this slice.
- Non-production Supabase has fifteen applied repository migrations, 418 canonical answers/801 aliases, seventeen narrow gameplay/status/discovery/draft RPC signatures plus one Realtime authorization helper, zero direct application-table grants, deny-all claim/catalog/draft tables, and private member-scoped per-player topics.
- `pnpm lint`, `pnpm typecheck`, `pnpm test` (114 tests across 20 files), `pnpm build`, `pnpm audit:client-bundle`, and `git diff --check` pass. All five direct hostile-client suites pass with Preview-scoped public configuration.
...
- Local desktop/mobile and protected Phase 7A discovery QA passed without horizontal overflow or application/runtime errors. Phase 7B local QA additionally passes reviewed chemical selection, name/symbol/alternate acceptance, generic results/coverage, and spoiler-safe sharing; its protected Preview passes homepage/practice/discovery HTTP, exact-source, runtime-log, and Production-isolation gates. Phase 7C1 local QA passes the complete owner draft lifecycle, locked submission state, mobile stacking, console, and overflow gates. Separate Phase 6 sessions previously passed the atomic elimination race/reveal gate.
...
The next slice is Phase 7C2: establish explicit reviewer authority and review outcome/version records before any publishing UI, then add category correction/version workflows, moderation/provenance administration, and abuse reporting. Preserve the discovery/draft boundaries, trusted daily/room identity, server clock, deny-all direct-table posture, active answer secrecy, private Realtime topics, atomic elimination claims, and Preview-only deployment policy. Do not fabricate activity, let ordinary anonymous users act as reviewers, admit unreviewed drafts to play, merge `main`, deploy Production, or configure CAPTCHA without explicit authorization and provider credentials.

...
| 7. General category studio and discovery | **In progress** | Phases 7A–7C4 ship editable discovery, reviewed general practice, private drafts, scope review, versioned banks, and independent bank decisions; publishing, moderation, and public lifecycle workflows remain. |
...
**Status: In progress.** Phases 7A–7C4 are published and Preview-verified. Phase 7C4 is complete on `codex/phase-7c4-answer-bank-decisions`, including its non-production Supabase migration and exact-source protected Preview. This phase makes the product broadly category-first without allowing unreviewed generation into competitive play.
...
- Treat assisted or AI-produced answer banks as untrusted drafts. Require moderation and a separate publishing step after bank approval. Unpublished drafts remain unavailable to practice and cannot enter daily leaderboards, public recommendations, or competitive rooms.
...
- Non-production Supabase has nineteen applied repository migrations, 418 canonical answers/801 aliases, twenty-seven narrow gameplay/status/discovery/draft/review/bank RPC signatures plus one Realtime authorization helper, zero direct application-table grants, deny-all claim/catalog/draft/reviewer/bank/review tables, an empty reviewer allowlist and empty bank-review tables after QA, and private member-scoped per-player topics.
- `pnpm lint`, `pnpm typecheck`, `pnpm test` (144 tests across 26 files), `pnpm build`, `pnpm audit:client-bundle`, and `git diff --check` pass. Existing direct hostile-client suites and the Phase 7C4 bank-decision hostile suite pass without printing local credential values.
...
- Local desktop/mobile and protected Phase 7A discovery QA passed without horizontal overflow or application/runtime errors. Phase 7B local QA additionally passes reviewed chemical selection, name/symbol/alternate acceptance, generic results/coverage, and spoiler-safe sharing; its protected Preview passes homepage/practice/discovery HTTP, exact-source, runtime-log, and Production-isolation gates. Phase 7C1 local QA passes the complete owner draft lifecycle. Phase 7C2 local QA passes ordinary-user denial and explicit reviewer queue/decision behavior. Phase 7C3 local QA passes bank open/validate/freeze/lock/correction. Phase 7C4 local QA passes the independent approval flow, concept fidelity, mobile action order, console, and overflow gates; its exact Preview passes protected page/API and runtime gates. Separate Phase 6 sessions previously passed the atomic elimination race/reveal gate.
...
The next slice is Phase 7C5: design a separate publishing transition that can project an approved private bank revision into immutable category/version data, reviewed discovery, and explicitly noncompetitive practice without granting ranked eligibility. Preserve the discovery/draft/reviewer/bank-decision boundaries, trusted daily/room identity, server clock, deny-all direct-table posture, active answer secrecy, private Realtime topics, atomic elimination claims, and Preview-only deployment policy. Do not fabricate activity, let ordinary anonymous users publish, treat bank approval as publication, admit unpublished banks to play, grant competitive eligibility implicitly, merge `main`, deploy Production, or configure CAPTCHA without explicit authorization and provider credentials.

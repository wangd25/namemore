...
- Phase 7A is implemented on `codex/phase-7-category-discovery`. One forward-only migration adds the curated discovery catalog, private owned draft storage, real thresholded ambient aggregates, hourly draft limiting, and two narrow authenticated RPCs. The migration is applied to non-production Supabase; local 1440×1000 and 390×844 discovery/search/draft/daily QA, advisors, grants, lint, typecheck, 103 tests, build, bundle audit, and whitespace checks pass. Publication and protected Preview verification remain open.
...
- Non-production Supabase project `namemore` (`hutmxxlicxeaovoeqbwg`) has anonymous sign-in enabled and thirteen applied repository-owned migrations. Phase 7A adds deny-all catalog/draft data and two category RPCs without changing the trusted game tables.
- The database contains 300 canonical answers and 561 normalized aliases in the unexposed `private` schema. All twelve application tables have RLS enabled and no permissive direct browser policies. Direct table privileges are revoked; fourteen gameplay/status/discovery RPC signatures plus one Realtime authorization helper enforce `auth.uid()`, membership, ownership, deadlines, atomic uniqueness/capacity/claims, immutable names, host authority, derived scores, active-answer secrecy, safe discovery, and private draft state.
...
- `pnpm test` passes 103 tests across 19 files. `pnpm lint`, `pnpm typecheck`, `pnpm build`, `pnpm audit:client-bundle`, and `git diff --check` pass. All four existing public-key hostile scripts pass with Preview-scoped public configuration. Phase 7A local browser QA covers desktop/mobile discovery, honest eligibility, search, draft navigation, daily regression, console, and overflow; its protected Preview gate is pending.
...
- NameMore is now category-first at entry: the homepage headline is an editable reviewed-catalog composer with a distinct private draft path. NBA remains the only playable reviewed vertical slice until Phase 7B adds a fully reviewed unrelated bank and generalized practice results.
...
| 7. General category studio and discovery | **In progress** | Phase 7A ships the editable catalog, honest review states, real aggregate cards, and private drafts; category-agnostic gameplay and reviewed unrelated banks remain. |
...
**Status: In progress.** Phase 7A is implemented on `codex/phase-7-category-discovery` and verified locally plus against the non-production Supabase project. Publication and protected Preview verification remain open. This phase makes the product broadly category-first without allowing unreviewed generation into competitive play.
...
Phase 7A verification: `pnpm test` passes 103 tests across 19 files; lint, strict typecheck, production build, client-bundle audit, and `git diff --check` pass. The thirteenth non-production migration is applied; catalog rows, answer counts, grants, safe search paths, authenticated execution, RLS, and advisors were inspected. Desktop/mobile browser QA passed real discovery, search, selection, draft navigation, `/daily` regression, console, and overflow checks. Remaining gate work is reviewed multi-subject answer banks, generalized category/game/result contracts, draft editing/review/publishing, correction/moderation/abuse workflows, hostile-client draft ownership/rate-limit coverage, branch publication, and protected Preview verification.
...
- Phase 7A is implemented on `codex/phase-7-category-discovery`; publication and protected Preview verification are pending.
- Non-production Supabase has thirteen applied repository migrations, fourteen narrow gameplay/status/discovery RPC signatures plus one Realtime authorization helper, zero direct application-table grants, deny-all claim/catalog/draft tables, and private member-scoped per-player topics.
- `pnpm lint`, `pnpm typecheck`, `pnpm test` (103 tests across 19 files), `pnpm build`, `pnpm audit:client-bundle`, and `git diff --check` pass. All four existing direct hostile-client suites pass with Preview-scoped public configuration.
...
- Desktop/mobile discovery QA passed without horizontal overflow or application browser errors. The real catalog/aggregate payload, debounced search, in-review play block, draft navigation, and `/daily` regression all passed; separate Phase 6 sessions previously passed the atomic elimination race/reveal gate.
...
The next slice is Phase 7B: add a fully reviewed unrelated category bank and generalize local practice/game results so NBA team metadata is optional. Preserve the new discovery/draft boundaries, trusted daily/room identity, server clock, deny-all direct-table posture, active answer secrecy, private Realtime topics, atomic elimination claims, and Preview-only deployment policy. Do not fabricate activity, admit unreviewed drafts to competitive play, merge `main`, deploy Production, or configure CAPTCHA without explicit authorization and provider credentials.

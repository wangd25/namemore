...
**Status: In progress.** Phases 7A–7C3 are published and Preview-verified. Phase 7C3 is complete on `codex/phase-7c3-answer-bank-versions`, including its non-production Supabase migrations and exact-source protected Preview. This phase makes the product broadly category-first without allowing unreviewed generation into competitive play.
...
- Treat assisted or AI-produced answer banks as untrusted drafts. Require an independent bank decision after freeze, moderation, and a separate publishing step. Unreviewed drafts remain unavailable to practice and cannot enter daily leaderboards, public recommendations, or competitive rooms.
...
- Non-production Supabase has eighteen applied repository migrations, 418 canonical answers/801 aliases, twenty-five narrow gameplay/status/discovery/draft/review/bank RPC signatures plus one Realtime authorization helper, zero direct application-table grants, deny-all claim/catalog/draft/reviewer/bank tables, an empty reviewer allowlist after QA, and private member-scoped per-player topics.
- `pnpm lint`, `pnpm typecheck`, `pnpm test` (135 tests across 24 files), `pnpm build`, `pnpm audit:client-bundle`, and `git diff --check` pass. Existing direct hostile-client suites and the Phase 7C3 rollback-only live integration pass without loading local credential files.
...
- Local desktop/mobile and protected Phase 7A discovery QA passed without horizontal overflow or application/runtime errors. Phase 7B local QA additionally passes reviewed chemical selection, name/symbol/alternate acceptance, generic results/coverage, and spoiler-safe sharing; its protected Preview passes homepage/practice/discovery HTTP, exact-source, runtime-log, and Production-isolation gates. Phase 7C1 local QA passes the complete owner draft lifecycle. Phase 7C2 local QA passes ordinary-user denial and explicit reviewer queue/decision behavior. Phase 7C3 local QA passes bank open/validate/freeze/lock/correction, concept fidelity, mobile action order, console, and overflow gates; its exact Preview passes ordinary-user and runtime gates. Separate Phase 6 sessions previously passed the atomic elimination race/reveal gate.
...
The next slice is Phase 7C4: design independent decisions for frozen answer-bank revisions—request correction, reject, or approve the bank—while still withholding every public publishing control. Preserve the discovery/draft/reviewer/bank boundaries, trusted daily/room identity, server clock, deny-all direct-table posture, active answer secrecy, private Realtime topics, atomic elimination claims, and Preview-only deployment policy. Do not fabricate activity, let ordinary anonymous users act as reviewers, treat scope approval or a frozen snapshot as published approval, admit unreviewed banks to play, merge `main`, deploy Production, or configure CAPTCHA without explicit authorization and provider credentials.

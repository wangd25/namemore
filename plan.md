...
- Phase 7C6 is complete on `codex/phase-7c6-practice-supersession`. One forward-only migration adds deny-all correction requests, immutable publication provenance links, a publisher correction RPC, and successor-aware queue, revision, and publication functions. The current version stays live until the original editor's copied revision passes freeze, independent approval, and independent publication; the successor preserves the slug, advances discovery atomically, retains every predecessor, and remains unranked. Application commit `5c2f16e197cb312de6205f0b412a1db7ed684170` is pushed; exact-source protected Git Preview `dpl_2Npqt8hpajWhM1fv7KKFE5D2XYDe` is `READY`, target null, and matches that commit. All 157 tests, hostile-client checks, live non-production inspection, desktop/mobile release-ledger QA, protected HTTP/runtime gates, production build, client-bundle audit, and Production isolation pass.
...
**Status: In progress.** Phases 7A–7C6 are published and Preview-verified. Phase 7C6 is complete on `codex/phase-7c6-practice-supersession`, including its non-production Supabase migration and exact-source protected Preview. This phase makes the product broadly category-first without allowing unreviewed generation into competitive play.
...
- [x] Add bounded reporting and moderation for reviewed categories: private reporter identity, one pending report per category version, a five-per-day reporter limit, explicit moderator authorization, dismiss-or-publisher-review outcomes, sanitized publisher escalation, and no automatic takedown, answer-bank mutation, ranking change, or competitive eligibility change. Wider anonymous platform controls remain a Phase 8 launch-hardening task.
...
Phase 7C6 verification: `pnpm test` passes 157 tests across 29 files; lint, strict typecheck, production build, client-bundle audit, and `git diff --check` pass. The twenty-first migration is applied to non-production Supabase with the baseline 418 answers/801 aliases unchanged after QA, one new private deny-all correction-request table, successor provenance constraints, and authenticated-only correction/supersession execution. The expanded public-key hostile suite passes direct-table and unauthorized-role denial, publisher-only correction, editor-only copied revision, independent reapproval, exact-slug enforcement, atomic version-2 publication, immutable version-1 counts, current practice resolution, and permanent noncompetitive state. Local real-backend QA passes the two-release ledger and correction-pending flow at 1440×1000 and 390×844 with accepted-concept fidelity, no console errors, and no horizontal overflow. All fixtures and temporary role entries were removed, returning the database to 2 category versions, 418 answers, 801 aliases, and 3 discovery items. Application commit `5c2f16e197cb312de6205f0b412a1db7ed684170` is pushed; exact-source protected Git Preview `dpl_2Npqt8hpajWhM1fv7KKFE5D2XYDe` at `https://namemore-4zyvoopwu-namemore.vercel.app` is `READY`, source `git`, target null, and matches that commit. Its publisher-page HTTP 200, build-error, runtime-error, 5xx, and Production-isolation gates pass.

Phase 7C7 verification: `pnpm test` passes 166 tests across 32 files; lint, strict typecheck, production build, client-bundle audit, and `git diff --check` pass. The twenty-second migration is applied to non-production Supabase with the baseline 2 category versions, 418 answers, 801 aliases, and 3 discovery items unchanged after QA; the new moderator/report/decision tables are private, RLS-enabled, directly unreadable, and empty after fixture cleanup. The public-key hostile suite passes direct-table and outsider denial, null/invalid input rejection, duplicate-pending protection, explicit allowlisting, identity-free queue projection, stale-decision rejection, sanitized publisher escalation, and unchanged reviewed/practice/noncompetitive discovery. Local real-backend QA passes report submission, successful private receipt, authorized moderation, queue advancement, the 1280×720 desktop and 390×720 responsive layouts, no horizontal overflow, and no browser warnings/errors. Accepted desktop/mobile/dialog concepts were compared against the final screenshots at native size; the implementation preserves the three-region hierarchy, single-column mobile order, boundary copy, and primary actions while using NameMore's existing blue action color and denser short-viewport spacing. Application commit `32865ed0d2e70f6dbdd6b08d4eedd316e777de50` is pushed; exact-source protected Git Preview `dpl_hVFLHCabetfByj2EdQiSaPJKRZgw` at `https://namemore-3aez7s8vh-namemore.vercel.app` is `READY`, source `git`, target null, and matches that commit. Its safe ordinary-user API returned HTTP 200 with no reports, and build-error, runtime-error, 5xx, and Production-isolation gates pass.
...
- Phase 7C6 is complete on `codex/phase-7c6-practice-supersession`; application commit `5c2f16e197cb312de6205f0b412a1db7ed684170` is pushed and exact-source protected Git Preview `dpl_2Npqt8hpajWhM1fv7KKFE5D2XYDe` is `READY` with a null Production target.
...
The next authorized milestone is Phase 8 production hardening. Begin with deployment-layer abuse controls and documented retention for anonymous identities and abandoned rooms, then complete the accessibility, privacy, dependency, RLS, authorization, release, rollback, and Production-configuration reviews. Preserve immutable release provenance, private reporter identity and notes, the discovery/draft/reviewer/bank/publisher/moderator boundaries, trusted daily/room identity, server clocks, deny-all direct-table posture, active answer secrecy, private Realtime topics, and atomic elimination claims. Do not merge `main`, deploy Production, configure CAPTCHA, purchase services, or change Production variables without explicit authorization.

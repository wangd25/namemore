...
- Phase 7B is implemented on `codex/phase-7b-general-practice`. A fourteenth forward-only migration adds optional category-neutral answer visual/group metadata and the reviewed noncompetitive 118-answer/240-alias Chemical Elements version. The static practice route, category-specific input/source copy, symbol rendering, generic coverage/results/sharing, local desktop/mobile QA, 110 tests, lint, typecheck, build, homepage bundle audit, live grants/count inspection, and Production isolation pass. Publication and protected Preview verification remain open.
...
- Non-production Supabase project `namemore` (`hutmxxlicxeaovoeqbwg`) has anonymous sign-in enabled and fourteen applied repository-owned migrations. Phase 7B adds 118 reviewed Chemical Elements answers and 240 aliases while keeping the category noncompetitive and direct application-table access denied.
...
- `pnpm test` passes 110 tests across 20 files. `pnpm lint`, `pnpm typecheck`, `pnpm build`, `pnpm audit:client-bundle`, and `git diff --check` pass. All four existing public-key hostile scripts pass with Preview-scoped public configuration. Phase 7A protected QA and Phase 7B local browser QA cover desktop/mobile discovery, honest eligibility, search, chemical play/results, aliases, generic coverage, daily regression, console/runtime logs, and overflow.
...
| 7. General category studio and discovery | **In progress** | Phases 7A–7B ship editable discovery, real aggregate cards, private drafts, generic practice/results, and a reviewed unrelated bank; moderation and lifecycle workflows remain. |
...
**Status: In progress.** Phase 7A is published and Preview-verified. Phase 7B is implemented on `codex/phase-7b-general-practice`, locally verified, and applied to the non-production Supabase project; publication and protected Preview verification remain open. This phase makes the product broadly category-first without allowing unreviewed generation into competitive play.
...
- Phase 7B is implemented on `codex/phase-7b-general-practice`; publication and protected Preview verification remain open.
- Non-production Supabase has fourteen applied repository migrations, 418 canonical answers/801 aliases, fourteen narrow gameplay/status/discovery RPC signatures plus one Realtime authorization helper, zero direct application-table grants, deny-all claim/catalog/draft tables, and private member-scoped per-player topics.
- `pnpm lint`, `pnpm typecheck`, `pnpm test` (110 tests across 20 files), `pnpm build`, `pnpm audit:client-bundle`, and `git diff --check` pass. All four existing direct hostile-client suites pass with Preview-scoped public configuration.

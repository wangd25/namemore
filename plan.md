...
- Phase 7C5 implementation and non-production verification are complete on `codex/phase-7c5-approved-bank-publishing`. One forward-only migration adds a deny-all publisher allowlist, immutable publication snapshots, and four narrow authenticated RPCs for status, queue, atomic reviewed-practice publication, and dynamic practice projection. The owner, bank editor, and approving reviewer cannot publish their revision. 155 tests, hostile-client checks, live inspection, and the real desktop/mobile publish → discovery → dynamic-practice flow pass; publication and Preview verification are pending.
...
| 7. General category studio and discovery | **In progress** | Phases 7A–7C5 ship editable discovery, reviewed general practice, private drafts, scope/bank review, and approved-bank publishing into unranked practice; correction, moderation, and public lifecycle workflows remain. |
...
**Status: In progress.** Phases 7A–7C4 are published and Preview-verified. Phase 7C5 implementation and non-production verification are complete on `codex/phase-7c5-approved-bank-publishing`; branch publication and exact-source protected Preview remain. This phase makes the product broadly category-first without allowing unreviewed generation into competitive play.
...
- Phase 7C5 implementation and non-production verification are complete on `codex/phase-7c5-approved-bank-publishing`; commit, push, and exact-source protected Preview verification remain.
- Non-production Supabase has twenty applied repository migrations, 418 canonical answers/801 aliases, thirty-one narrow gameplay/status/discovery/draft/review/bank/publishing RPC signatures plus one Realtime authorization helper, zero direct application-table grants, deny-all claim/catalog/draft/reviewer/bank/review/publishing tables, empty reviewer/publisher allowlists and bank/review/publication tables after QA, and private member-scoped per-player topics.
- `pnpm lint`, `pnpm typecheck`, `pnpm test` (155 tests across 29 files), `pnpm build`, `pnpm audit:client-bundle`, and `git diff --check` pass. Existing direct hostile-client suites and the expanded category-bank publishing hostile suite pass without printing local credential values.
...
The next slice after Phase 7C5 publication is Phase 7C6: design correction/supersession and provenance-administration workflows for already published practice versions without mutating history or silently changing competitive eligibility. Preserve the discovery/draft/reviewer/bank/publisher boundaries, trusted daily/room identity, server clock, deny-all direct-table posture, active answer secrecy, private Realtime topics, atomic elimination claims, and Preview-only deployment policy. Do not fabricate activity, let ordinary anonymous users publish or overwrite history, treat bank approval as publication, grant competitive eligibility implicitly, merge `main`, deploy Production, or configure CAPTCHA without explicit authorization and provider credentials.

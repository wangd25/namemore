...
- Phase 7C2 is complete locally on `codex/phase-7c2-reviewer-authority`. A sixteenth forward-only migration adds a private explicit reviewer allowlist, append-only revision snapshots, owner-visible decision outcomes, request-changes/resubmission, and terminal scope-approved/rejected states without publishing or eligibility. The private reviewer workspace, no-store handlers, 124 tests, category-review hostile-client suite, live schema/count/advisor inspection, and local desktop/mobile browser flow pass; exact-source Preview publication remains the final gate.
...
- Non-production Supabase project `namemore` (`hutmxxlicxeaovoeqbwg`) has anonymous sign-in enabled and fifteen applied repository-owned migrations. Phase 7B adds 118 reviewed Chemical Elements answers and 240 aliases while keeping the category noncompetitive and direct application-table access denied; Phase 7C1 adds only private draft lifecycle state and owner operations.
- The database contains 418 canonical answers and 801 normalized aliases in the unexposed `private` schema. All twelve application tables have RLS enabled and no permissive direct browser policies. Direct table privileges are revoked; seventeen gameplay/status/discovery/draft RPC signatures plus one Realtime authorization helper enforce `auth.uid()`, membership, ownership, deadlines, atomic uniqueness/capacity/claims, immutable names, host authority, derived scores, active-answer secrecy, safe discovery, and private draft state.
...
- `pnpm test` passes 114 tests across 20 files. `pnpm lint`, `pnpm typecheck`, `pnpm build`, `pnpm audit:client-bundle`, and `git diff --check` pass. Five public-key hostile scripts pass with Preview-scoped public configuration. Phase 7C1 local browser QA covers owner draft creation/edit/save/submit/lock behavior, desktop/mobile layout, console diagnostics, and overflow.
...
| 7. General category studio and discovery | **In progress** | Phases 7A–7C2 ship editable discovery, real aggregates, reviewed general practice, owner drafts, and explicit reviewer outcomes; answer-bank review, publishing, moderation, corrections, and lifecycle workflows remain. |
...
**Status: In progress.** Phases 7A, 7B, and 7C1 are published and Preview-verified. Phase 7C2 is complete locally on `codex/phase-7c2-reviewer-authority`, including its non-production Supabase migration; exact-source protected Preview publication remains. This phase makes the product broadly category-first without allowing unreviewed generation into competitive play.
...
Phase 7C2 local verification: `pnpm test` passes 124 tests across 22 files; lint and strict typecheck pass. The sixteenth migration is applied to non-production Supabase with 418 answers/801 aliases unchanged, three authenticated reviewer RPCs, two private deny-all reviewer tables, zero invalid pending revisions, and only the expected advisor notices. The category-review hostile-client suite passes explicit allowlisting, direct-table and outsider denial, self-review prevention, immutable revision snapshots, request-changes/resubmission, owner-visible reasons without reviewer identity, noncompetitive scope approval, and discovery privacy. Local in-app-browser QA passes ordinary-user denial, authorized queue/decision advancement, desktop/mobile concept fidelity, console, and overflow gates.
...
- Phase 7C2 is complete locally on `codex/phase-7c2-reviewer-authority`; its sixteenth migration, reviewer UI, owner outcome states, and hostile-client/browser gates pass while publication is pending.
- Non-production Supabase has sixteen applied repository migrations, 418 canonical answers/801 aliases, twenty narrow gameplay/status/discovery/draft/review RPC signatures plus one Realtime authorization helper, zero direct application-table grants, deny-all claim/catalog/draft/reviewer tables, and private member-scoped per-player topics.
- `pnpm lint`, `pnpm typecheck`, and `pnpm test` (124 tests across 22 files) pass. The build, client-bundle, whitespace, and exact-source Preview publication gates are next. All six direct hostile-client suites pass with Preview-scoped public configuration.
...
- Local desktop/mobile and protected Phase 7A discovery QA passed without horizontal overflow or application/runtime errors. Phase 7B local QA additionally passes reviewed chemical selection, name/symbol/alternate acceptance, generic results/coverage, and spoiler-safe sharing; its protected Preview passes homepage/practice/discovery HTTP, exact-source, runtime-log, and Production-isolation gates. Phase 7C1 local QA passes the complete owner draft lifecycle. Phase 7C2 local QA passes ordinary-user denial, explicit reviewer queue/decision behavior, concept fidelity, mobile stacking, console, and overflow gates. Separate Phase 6 sessions previously passed the atomic elimination race/reveal gate.
...
The next slice is Phase 7C3: design the reviewed answer-bank correction/version and provenance workflow before any public publishing control, then add moderation administration and abuse reporting. Preserve the discovery/draft/reviewer boundaries, trusted daily/room identity, server clock, deny-all direct-table posture, active answer secrecy, private Realtime topics, atomic elimination claims, and Preview-only deployment policy. Do not fabricate activity, let ordinary anonymous users act as reviewers, treat scope approval as answer-bank approval, admit unreviewed drafts to play, merge `main`, deploy Production, or configure CAPTCHA without explicit authorization and provider credentials.
